...
static double sum;
static IntArr * arr2;
static DoubleArr * arr3;


void _arrFunc0(int x) {
{
sum = sum+(double)(x*multiplier);
;

}
}

void _arrFunc1(double x) {
{
sum = sum+x;
;
...
int main(){
int _arr[] = {1, 2, 3, 4, 5};;
arr = malloc(sizeof(IntArr));
arr->arr = _arr;
arr->size = NELEMS(_arr);
;
multiplier = 4;
sum = 0;
for(int __i = 0; __i < arr->size; __i++)
	_arrFunc0(arr->arr[__i]);
if (sum > 40){
_printIntArr(arr);
;

} else{
printf("%s\n","Sum by 4 not greater than 40");
;

}int _arr2[] = {1, 2, 3, 4, 5};;
arr2 = malloc(sizeof(IntArr));
arr2->arr = _arr2;
arr2->size = NELEMS(_arr2);
;
arr3 = _prodIntArrDouble(arr2,multiplier);
;
sum = 0;
for(int __i = 0; __i < arr3->size; __i++)
	_arrFunc1(arr3->arr[__i]);
if (sum > 40){
_printDoubleArr(arr3);
;
