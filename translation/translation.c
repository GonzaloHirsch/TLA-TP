...
char *processArrayAccess(GenericNode *gn)
{
    ...
    char * fmt = "*%s->on_access(%s, %s)";
    buffer = malloc(strlen(var) * 2 + strlen(indexExpr) + strlen(fmt) - 3 + 1);
    sprintf(buffer, fmt, var, var, indexExpr);
    return buffer;
}
